...
function failAfterStream(options) {
    ...
    return through.obj(function(chunk, enc, cb) {
        ...
        if (isStats && _.isUndefined(visited[stats.hash])) {
            visited[stats.hash] = stats;
        }
        ...
        if (options.errors === true) {
            hasErrors = _(visited).chain().values().some(function(x) {
                return x.hasErrors();
            }).value();
        }

        if (options.warnings === true) {
            hasWarnings = _(visited).chain().values().some(function(x) {
